...
pca_variance = 0.9  # Keep 90% variance

# 1. Load the PSD features (EEG only)
psd_path = os.path.join(data_root, f"all_subjects_{window_size}windowed_features.csv")
if not os.path.exists(psd_path):
    raise FileNotFoundError(f"Could not find PSD file at {psd_path}")

df_psd = pd.read_csv(psd_path).dropna().reset_index(drop=True)

# Drop eye-tracking features (anything from fix_count onwards)
if any(df_psd.columns.str.contains("fix_count")):
    idx_fix = np.where(df_psd.columns.str.contains("fix_count"))[0][0]
    cols_to_keep = ["subject_id", "label"] + list(df_psd.columns[:idx_fix])
    cols_to_keep = list(dict.fromkeys(cols_to_keep)) 
    df_psd = df_psd[cols_to_keep]

# Create helper index for 1-to-1 matching
df_psd['sample_in_sub'] = df_psd.groupby(['subject_id', 'label']).cumcount()

# 2. Collect and prepare PLV features
all_subjects = sorted(
...
for subject_id in all_subjects:
    feature_file = os.path.join(data_root, subject_id, "ml_data", 
    ...
    if os.path.exists(feature_file):
        ...
        # Helper index must be added to EACH subject before concatenation
        df_sub['sample_in_sub'] = df_sub.groupby(['subject_id', 'label']).cumcount()
        plv_dfs.append(df_sub)
    else:
        print(f"Skipping {subject_id}: No PLV file.")

df_plv = pd.concat(plv_dfs, ignore_index=True)

# 3. Final Merge: PSD + PLV
df = pd.merge(
    df_psd, 
    df_plv, 
    on=["subject_id", "label", "sample_in_sub"], 
    how="inner"
).drop(columns=['sample_in_sub'])

print(f"Merged Data Shape: {df.shape}")
print(f"Total Features: {len(df.columns)-2} (PSD + PLV combined)")

# ==========================
# Features & Groups for LOSO
# ==========================
...
pred_file = os.path.join(
    results_dir,
    f"loso_predictions_{window_size}win_eeg.csv",
)
...
metrics_file = os.path.join(
    results_dir,
    f"loso_metrics_{window_size}win_eeg.csv",
)
df_metrics.to_csv(metrics_file, index=False)

print(f"\nSaved aggregated metrics to {metrics_file}")
print("\n=== Overall summary ===")
print(df_metrics.sort_values(["accuracy"], ascending=[False]))
...
subj_metrics_file = os.path.join(
    results_dir,
    f"loso_subject_metrics_{window_size}win_eeg.csv",
)
