...
def eval_model(dataloaders, device, tokenizer, criterion, model, output_all_results_path='./results/temp.txt'):
    ...
    os.makedirs(os.path.dirname(output_all_results_path) or ".", exist_ok=True)
...
if __name__ == '__main__':
    ...
    dataset_type = training_config['dataset_type']
    ...
    print(f'teacher_forcing{teacher_forcing} intput_noise{intput_noise}')
    output_all_results_path = (f'./results/{dataset_type}-{model_name}{"-teacher_forcing" if teacher_forcing else ""}{"-intput_noise" if intput_noise else ""}-all_decoding_results.txt')
    ...
    # test dataset
    test_set = ROAMMEEG2TextDataset(csv_path=csv_path, data_type=dataset_type, split='test', tokenizer=tokenizer,
    ...
    if model_name == 'BrainTranslator':
        model = BrainTranslator(pretrained_bart, in_feature = IN_FEATURE, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
    elif model_name == 'BrainTranslatorNaive':
        model = BrainTranslatorNaive(pretrained_bart, in_feature = IN_FEATURE, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
