...
from data_roamm import ROAMMEEG2TextDataset
from model_decoding import BrainTranslator, BrainTranslatorNaive
from config import get_config


def train_model(dataloaders, device, model, criterion, optimizer, scheduler, num_epochs=50,
...
if __name__ == '__main__':
    ...
    if skip_step_one:
        save_name = f'{dataset_type}_finetune_{model_name}_skipstep1_b{batch_size}_{num_epochs_step1}_{num_epochs_step2}_{step1_lr}_{step2_lr}_{dataset_setting}'
    else:
        save_name = f'{dataset_type}_finetune_{model_name}_2steptraining_b{batch_size}_{num_epochs_step1}_{num_epochs_step2}_{step1_lr}_{step2_lr}_{dataset_setting}'
    ...
    csv_path = args.get('csv_path', '/gpfs1/pi/anon/mindless_reading/data/all_subjects_eeg2text_data_weightedavg.csv')
    ...
    max_target_len = args.get('max_target_len', 64)    # number of text tokens

    print(f'[INFO]CSV path: {csv_path}')
    print(f'[INFO]max_seq_len={max_seq_len}, max_target_len={max_target_len}')

    # Tokenizer already set below (BART for BrainTranslator*)
    ...
    # train dataset
    train_set = ROAMMEEG2TextDataset(csv_path=csv_path, data_type=dataset_type, split='train', tokenizer=tokenizer,
                                   max_seq_len=max_seq_len, max_target_len=max_target_len)
    # dev dataset
    dev_set = ROAMMEEG2TextDataset(csv_path=csv_path, data_type=dataset_type, split='dev', tokenizer=tokenizer,
                                   max_seq_len=max_seq_len, max_target_len=max_target_len)
    ...
    IN_FEATURE = 64 * 8  # 512 = 64 channels x 8 bands
    ...
    if model_name == 'BrainTranslator':
        ...
        model = BrainTranslator(pretrained, in_feature = IN_FEATURE, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
    
    elif model_name == 'BertGeneration':
        pretrained = BertLMHeadModel.from_pretrained('bert-base-cased', config=config)
        model = BrainTranslator(pretrained, in_feature = IN_FEATURE, decoder_embedding_size = 768, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
    elif model_name == 'BrainTranslatorNaive':
        pretrained = BartForConditionalGeneration.from_pretrained('facebook/bart-large')
        model = BrainTranslatorNaive(pretrained, in_feature = IN_FEATURE, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048)
